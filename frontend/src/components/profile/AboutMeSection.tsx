...
export default function About({ user, onSaved }: AboutProps) {
  ...
  function startEdit() {
    ...
    setFavoriteSubjectsText(user.favoriteSubjects.join(", "));
    setError(null);
    setEditing(true);
  }
  ...
  async function save() {
    ...
    setError(null);
    try {
      const favoriteSubjects = favoriteSubjectsText
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
      const res = await fetch(`${API_BASE}/profile`, {
        ...
        body: JSON.stringify({ username, aboutMe, favoriteSubjects }),
      });
      if (!res.ok) {
      ...
      <div id="top-row" className="flex items-center justify-between w-full mb-4 mt-4">
        {editing ? (
          <input
            ...
            className="text-3xl font-semibold jersey-25-regular bg-white px-2 py-1 rounded ring-1 ring-foreground/20"
          ...
          <p className="text-xl text-black mb-2 jersey-25-regular">Favorite Subjects:
            {editing ? (
              <input
                ...
                onChange={(e) => setFavoriteSubjectsText(e.target.value)}
                placeholder="Comma-separated, e.g. History, Computer Science"
                className="block w-full mt-2 mb-4 text-sm bg-white px-2 py-1 rounded ring-1 ring-foreground/20"
              />
            ) : (
